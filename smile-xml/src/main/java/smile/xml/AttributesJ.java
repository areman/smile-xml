...
public class AttributesJ extends BaseJ<Node>
{
  ...
  public static RubyClass define(Ruby runtime)
  {
    ...
    RubyClass result = module.defineClassUnder("Attributes", runtime.getObject(), ALLOCATOR);
  ...
  private static RubyClass getRubyClass(Ruby runtime) {
    return UtilJ.getClass(runtime, new String[] { "LibXML", "XML", "Attributes" });
  }

  public static AttributesJ newInstance(ThreadContext context) {
    return (AttributesJ)getRubyClass(context.getRuntime()).newInstance(context, new IRubyObject[0], null);
  }
  ...
  public IRubyObject getValue(ThreadContext context, IRubyObject pName)
  {
    ...
    Node node = ((Node)getJavaObject()).getAttributes().getNamedItem(name.asJavaString());
  ...
  @JRubyMethod(name={"[]="})
  public void setValue(ThreadContext context, IRubyObject pName, IRubyObject pValue) {
    if ((pName instanceof RubySymbol)) {
      pName = pName.asString();
    }
    ...
    Node node = ((Node)getJavaObject()).getAttributes().getNamedItem(name.asJavaString());
    if (node == null) {
      node = ((Node)getJavaObject()).getOwnerDocument().createAttribute(name.asJavaString());
  ...
  public IRubyObject getAttributeNs(ThreadContext context, RubyString namespace, RubyString name) {
    Node node = ((Node)getJavaObject()).getAttributes().getNamedItemNS(namespace.asJavaString(), name.asJavaString());
    if (node != null) {
      AttrJ attr = AttrJ.newInstance(context);
      attr.setJavaObject(node);
      return attr;
  ...
  public RubyFixnum getLength(ThreadContext context) {
    int r = ((Node)getJavaObject()).getAttributes() == null ? 0 : ((Node)getJavaObject()).getAttributes().getLength();
  ...
  public RubyHash toHash(ThreadContext context) {
    ...
    for (int i = 0; i < ((Node)getJavaObject()).getAttributes().getLength(); i++) {
      Node a = ((Node)getJavaObject()).getAttributes().item(i);
      map.put(context.getRuntime().newString(a.getNodeName()), context.getRuntime().newString(a.getNodeValue()));
    }
